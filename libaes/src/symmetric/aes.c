...
void ExpandKey(const uint8_t *cipherKey, aes_ctx_st* ctx) {
	uint32_t *rek = ctx->e_sched;
	uint32_t i, k, n;
	uint32_t temp;
	uint32_t r = 0;

	for (i = 0, k = 0; i < ctx->Nk; i++, k += 4) {
		rek[i] =
			((cipherKey[k    ] & 0xff) << 24) |
	...
	for (i = ctx->Nk, n = 0; i < ctx->Nw; i++, n--) {
		temp = rek[i - 1];
		if (n == 0) {
			n = ctx->Nk;
			temp =
				(Te4[(temp >> 16) & 0xff] & 0xff000000) |  // Removed one >
				(Te4[(temp >>  8) & 0xff] & 0x00ff0000) |
				(Te4[(temp       ) & 0xff] & 0x0000ff00) |
				(Te4[(temp >> 24)  & 0xff] & 0x000000ff);
			temp ^= rcon[r++];
		} else if (ctx->Nk == 8 && n == 4) {
			temp =
		...
		rek[i] = rek[i - ctx->Nk] ^ temp;
	}
	temp = 0;
}

void InvertKey(aes_ctx_st *ctx) {
	uint32_t *rek = ctx->e_sched;
	uint32_t *rdk = ctx->d_sched;
	uint32_t r;
...
errno_t makeKey(const uint8_t *cipherKey, uint16_t keySize, uint8_t dir, aes_ctx_st *ctx) {
	errno_t result;

	if(cipherKey == NULL || ctx == NULL) {
	...
	ctx->keysize = keySize;
	ctx->direction = dir;
	ctx->Nk = keySize >> 5; // Removed one >
	ctx->Nr = ctx->Nk + 6;
	ctx->Nw = 4*(ctx->Nr + 1);
	//assert(dir >= DIR_NONE && dir <= DIR_BOTH);
	if (dir == DIR_ENCRYPTION || dir == DIR_DECRYPTION) {
		ExpandKey(cipherKey, ctx);
		if (dir & DIR_DECRYPTION) {
			InvertKey(ctx);
		}
	}
...
static void encrypt_in(const uint8_t *pt, uint8_t *ct, aes_ctx_st *ctx) {
	uint32_t *rek = ctx->e_sched;
...
static void decrypt_in(const uint8_t *ct, uint8_t *pt, aes_ctx_st *ctx) {
	uint32_t *rdk = ctx->d_sched;
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
...
errno_t aesInit(uint8_t *key, uint16_t keySize, uint8_t dir, aes_ctx_st* ctx) {
	return makeKey(key, keySize, dir, ctx);
}
...
errno_t aesClearContext(aes_ctx_st *ctx) {
	...
	result = memset_s(ctx, sizeof(aes_ctx_st), 0, sizeof(aes_ctx_st));
FAIL:
	return result;
}
...
errno_t aesProcessBlock(const uint8_t* input, uint8_t* output, void *ctx) {
	...
	result = aesCheckContext(aes_ctx);
	...
	if(aes_ctx->direction == DIR_ENCRYPTION) {
		encrypt_in(input, output, aes_ctx);
	} else if(aes_ctx->direction == DIR_DECRYPTION) {
		decrypt_in(input, output, aes_ctx);
	}
